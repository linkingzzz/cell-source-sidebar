...
Excel 任务窗格加载项（Office.js）。侧边栏是**针对选中单元格**的批注，分三块：1 附件（上传 / 点击下载 / 删除）、2 明细表（9 × 4 可编辑表格）、3 备注，底部「保存到工作簿」写回。功能区页签「数据源」下有 **显示侧边栏批注** / **隐藏侧边栏批注** 两个按钮，与 `wps-plugin/` 同款、同存储契约。
...
- ✅ **已在真实 Excel 中实测通过**（Excel 16.0.20326.20132 / Microsoft 365 家庭版，Windows）：
  ...
  - 功能区「显示侧边栏批注」：关掉侧边栏后点它可重新打开。
  - 2026-09-12 复测（改共享运行时之后，截图 `../_附件/excel-v3-*.png`）：页签 `数据源`、按钮 `隐藏侧边栏批注`（悬停提示「隐藏右侧的批注侧边栏」）→ 点它侧边栏消失 → 再点「显示侧边栏批注」回来 → 备注保存后状态变「已有批注」。
...
- ⚠️ 已知观感差异：Excel 会把按钮标签折成两行（「显示侧 / 边栏批注」），WPS 同一标签是一行。功能不受影响。
